...
export async function POST(request: NextRequest) {
  try {
    ...
    const pythonScriptPath = path.join(process.cwd(), 'scripts', 'process_exam.py');
    ...
    if (result.processed_image_path) {
      const originalPath = path.join(process.cwd(), result.processed_image_path);
      const publicPath = path.join(PROCESSED_DIR, path.basename(result.processed_image_path));
      ...
      try {
        ...
        result.processed_image_path = `/processed_images/${path.basename(result.processed_image_path)}`;
      } catch (error) {
    ...
    const visualization = {
      ...
      detections: formatDetections(result.detections)
    };
    ...
    const responseData = {
      ...
      result: {
        ...
        excel_data: {
          metadata,
          students: formatStudentData(result.excel_data.students)
        },
        programme_verification: result.programme_verification || {
          match: true, // Default to true if not provided
          detected: programme,
          selected: programme
        }
      }
...
// Helper function to format detections data
function formatDetections(detections: string[]): any[] {
  // Convert the simple detections array to the required format
  return detections.map((className, index) => {
    ...
    const y2 = y1 + Math.floor(Math.random() * 100) + 50;
    
    return {
      class: className,
      coordinates: [x1, y1, x2, y2],
      confidence: 0.8 + Math.random() * 0.2,
      recognized_value: "Detected"
    };
  });
}

// Helper function to format student data
function formatStudentData(students: any[]): any[] {
  return students.map(student => {
    // Create a more structured format
    const questions: any = {};
    
    Object.keys(student.questions).forEach(qKey => {
      questions[qKey] = {};
      Object.keys(student.questions[qKey]).forEach(subKey => {
        questions[qKey][subKey] = student.questions[qKey][subKey] ? "1" : "0";
      });
    });
    
    return {
      usn: "USN12345", // Example USN
      total_marks: "85",  // Example total
      questions
    };
  });
}